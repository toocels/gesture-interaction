...
class hand_skeleton:
    ...
    def left_or_right(self, hand_coords):
...
class gesture_control:
    ...
    def relative_coords_cal(self, all_coords, extremes):
        ...
        gestures.relative_coords = np.array(
            gestures.relative_coords).reshape(1, -1)


# One Time Initialisations
cap = cv2.VideoCapture(0)

colours=["red","green","cyan","gray","white"]
...
while True:
    ...
    for hand in all_coords:  # if a hand is visibe
        ...
        gestures.Roi = img[extremes[1]-10:extremes[3] +
                           10, extremes[0]-10:extremes[2]+10]

        pickle_in = open(__file__+"\\..\\gestures.pickle", 'rb')
        model = pickle.load(pickle_in)

        prediction, confidence = model.predict(
            gestures.relative_coords), model.predict_proba(gestures.relative_coords)[0]

        cv2.putText(img_drawn, ["L","R"][skeleton.left_or_right(hand)],
                    hand[0], cv2.FONT_HERSHEY_PLAIN, 2, (255, 0, 255), 2)
        hand_side=skeleton.left_or_right(hand)
